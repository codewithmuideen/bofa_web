...
export default function DashboardPage() {
  ...
  const cards = [
    { label: "Checking Account", balance: bal.checking, account: user.accountNumber, bg: "#1C3668", tag: "Bank of America" },
    { label: "Savings Account", balance: bal.savings, account: user.accountNumber.replace("****", "****"), bg: "#0A1628", tag: "Advantage Savings" },
    { label: "Cash Rewards", balance: bal.rewards, account: "Rewards Points", bg: "#E31837", tag: "BankAmericard" },
  ];
  ...
  return (
    <AppShell>
      <div className="animate-fade-in">
        {/* Greeting */}
        <div className="bg-[#1C3668] px-4 pb-7 pt-3 flex items-center justify-between gap-3">
          ...
          <div className="flex gap-4 overflow-x-auto pb-2 snap-x snap-mandatory hide-scrollbar">
            {cards.map((card, idx) => (
              <div
                ...
                className={`card-premium snap-center shrink-0 w-[calc(100vw-48px)] max-w-sm rounded-3xl p-5 shadow-lift cursor-pointer transition-transform ${activeCard === idx ? "scale-100" : "scale-[0.97]"}`}
              >
        ...
        <div className="mt-6 px-4">
          <div className="bg-white rounded-2xl shadow-card p-5">
            ...
            <div className="flex items-end justify-between gap-2 h-20">
              {spendBars.map(t => (
                <div key={t.id} className="flex-1 h-full flex items-end group">
                  <div
                    className={`w-full rounded-lg transition-all group-hover:opacity-80 ${t.amount < 0 ? "bg-[#1C3668]" : "bg-emerald-500"}`}
            ...
            <div className="flex justify-between gap-2 mt-1.5">
              {spendBars.map(t => (
                <span key={t.id} className="flex-1 text-[9px] text-[#9CA3AF] truncate text-center">{t.merchant.split(" ")[0]}</span>
              ))}
            </div>
          </div>
        </div>
